...
$glb_debug = 0;
$glb_moddebug = 0;
# Turned off so that we can get Perl warnings without making bp
...
sub format {
  ...
  # XXXXX make sure this is ok.  What if Iformat is set to 'bibtex:troff' and
...
sub convert {
  ...
  # print STDERR "bp.pl convert: conv_func = " . (defined($conv_func) ? $conv_func : "<undef>") . "\n";
...
# print STDERR "calling converter '$glb_cvtname'\n";
    return &$func($recin);
  }
  ...
  # print STDERR "convert (2): conv_func = " . (defined($conv_func) ? $conv_func : "<undef>") . "\n";

  # print STDERR "recin: $recin\n";
  # print STDERR "explode: ", &explode($recin), "\n";
  # print STDERR "tocanon: ", &tocanon(&explode($recin)), "\n";
  # print STDERR "fromcanon: ", &fromcanon(&tocanon(&explode($recin))), "\n";
  # print STDERR "implode: ", &implode(&fromcanon(&tocanon(&explode($recin)))), "\n";
...
sub fromcanon {
  ...
  if (defined $file) {
    ...
    if (!defined($format)) {
      print STDERR "unopened file $file given to fromcanon\n";
    }
  ...
  %recexp = &$func(%record);
  # print STDERR "First, the format conversion: $func ", %record, "\n";
  # print STDERR "result = ", %recexp, "\n";
  ...
  # print STDERR "bp::fromcaonon => ", %recexp, "\n";
